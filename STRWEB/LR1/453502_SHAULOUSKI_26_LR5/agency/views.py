...
import io
import logging
import zoneinfo # Импортируем zoneinfo для работы с таймзонами
...
import io
import base64
import statistics  # Для автоматического расчета медианы и моды
from django.contrib.auth.decorators import login_required
from django.shortcuts import render
from django.db.models import Sum, Avg
from django.utils import timezone

# Переключение matplotlib в невизуальный режим (чтобы не падало на сервере без GUI)
import matplotlib
...
def dashboard_view(request):
    ...
    """
    logger.info("User %s (ID: %d) accessed the dashboard.", request.user.username, request.user.id)

    # 1. БЕЗОПАСНОЕ ОПРЕДЕЛЕНИЕ ЧАСОВОГО ПОЯСА (Исправлена ошибка отсутствия поля timezone)
    ...
    try:
        ...
        logger.error("Currency extraction failed: %s", e)
        rate = 3.25
    ...
    if hasattr(request.user, 'employeeprofile'):
        ...
        client_ids = deals_queryset.values_list('client_id', flat=True).distinct()
        clients_queryset = ClientProfile.objects.select_related('user').filter(id__in=client_ids)
        ...
        # Корректный подсчет типов сделок
        sales_count = deals_queryset.filter(deal_type='sale').count()
        rent_count = deals_queryset.filter(deal_type='rent').count()

        # Расчет медианы и моды
        prices_list = list(deals_queryset.values_list('final_price', flat=True))
        prices_in_byn = [float(p or 0) * rate for p in prices_list if p]

        median_sales_str = "—"
        mode_sales_str = "—"

        if prices_in_byn:
            median_val = statistics.median(prices_in_byn)
            median_sales_str = f"{median_val:,.0f}".replace(",", " ")
            try:
                mode_val = statistics.mode(prices_in_byn)
                mode_sales_str = f"{mode_val:,.0f}".replace(",", " ")
        ...
        for deal in deals_queryset:
            if deal.property and deal.property.prop_type:
                t_name = deal.property.prop_type.name
                type_counts[t_name] = type_counts.get(t_name, 0) + 1

        popular_type_str = max(type_counts, key=type_counts.get) if type_counts else "Нет данных"
        ...
        if type_counts:
            try:
                ...
                colors = ['#2b7a8d', '#e28743', '#28a745', '#9c27b0', '#ffc107']

                ax.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90,
                       colors=colors[:len(labels)], textprops={'fontsize': 10, 'color': '#2d3748'})
                ax.axis('equal')

                buf = io.BytesIO()
                plt.savefig(buf, format='png', bbox_inches='tight', transparent=True)
                buf.seek(0)
                chart_data_b64 = base64.b64encode(buf.getvalue()).decode('utf-8')
                buf.close()
                plt.close(fig)
            except Exception as chart_err:
                logger.error("Chart generation failed: %s", chart_err)

        # Подготовка данных сделок для вывода в шаблон
        for deal in deals_queryset:
            deal.amount = f"{float(deal.final_price or 0) * rate:,.0f}".replace(",", " ")
            deal.created_at_local = deal.created_at_utc.astimezone(user_tz)
            deal.timezone_name = user_tz_name
            # Добавляем понятный текст типа сделки для отображения
            deal.human_type = "Продажа" if deal.deal_type == 'sale' else "Аренда"

        context = {
            'deals': deals_queryset,
            'clients': clients_queryset,
            'total_sales': f"{float(raw_total) * rate:,.0f}".replace(",", " "),
            'avg_sales': f"{float(raw_avg) * rate:,.0f}".replace(",", " "),
            'median_sales': median_sales_str,
            'mode_sales': mode_sales_str,
            'avg_age': sales_count,
    ...
    # 2. CLIENT DASHBOARD INTERFACE
        ...
        try:
            ...
            client_profile, promo_codes, phone = None, [], "Не указан"
        ...
        for purchase in purchases_queryset:
            ...
            # Добавляем тип сделки и для клиента
            purchase.human_type = "Продажа" if purchase.deal_type == 'sale' else "Аренда"
...
def create_deal_ajax(request, property_id):
    ...
    if Deal.objects.filter(property=property_obj).exists():
        messages.error(request, f'Данная позиция («{property_obj.title}») уже участвует в сделки.')
        return redirect(next_url)
